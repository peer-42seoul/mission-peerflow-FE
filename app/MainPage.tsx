  ...
  const fetchData = async () => {
    try {
      let url = !(category === 'search')
        ? category
          ? `http://paulryu9309.ddns.net:80/v1?category=${category}&sort=${sort}&pagingIndex=${
              page[category ? category : 'main'] - 1
            }&pagingSize=${5}`
          : `http://paulryu9309.ddns.net:80/v1?sort=${sort}&pagingIndex=${
              page[category ? category : 'main'] - 1
            }&pagingSize=${5}`
        : `http://paulryu9309.ddns.net:80/v1/search?title=${title}&sort=${sort}`
